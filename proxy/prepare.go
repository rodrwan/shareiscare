...
func PrepareCloudflared() error {
	...
	default:
		return fmt.Errorf("SO no soportado")
	}
	...
	if err != nil {
		return fmt.Errorf("error al descargar el binario: %w", err)
	}
	...
	if goos == "darwin" {
		...
		if err != nil {
			return fmt.Errorf("error al verificar archivo .tgz: %w", err)
		}

		if fileInfo.Size() < 1000 {
			return fmt.Errorf("archivo .tgz demasiado pequeño, posiblemente corrupto: %d bytes", fileInfo.Size())
		}
		...
		if err := extractTarGz(outPath); err != nil {
			...
			if err != nil {
				return fmt.Errorf("error en descarga directa: %w", err)
			}
